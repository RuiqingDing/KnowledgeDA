...
from aug_quality_assessment import *
...
train(config, model, train_iter, dev_iter, learning_rate, f"save_dict/{config.dataname}_{config.datatype}_Filter_{config.aug_num}_{config.bert_path.split('/')[-1]}_{opt.seed}.ckpt")
print('train spending: ', get_time_dif(time_train_start))

time_test_start = time.time()
test(config, model, test_iter, f"save_dict/{config.dataname}_{config.datatype}_Filter_{config.aug_num}_{config.bert_path.split('/')[-1]}_{opt.seed}.ckpt")
